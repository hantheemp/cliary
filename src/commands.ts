...
function add(program: Command): void {
  program
    .command("add")
    .description(
      "Add a new diary entry. If no text is provided, system's default text editor will be opened.",
    )
    .argument("[note]", "The raw, unformatted text of the diary entry.")
    .option(
      "-t, --title <string>",
      "Set/overwrite today's title at the same time.",
    )
    .action((note: string | undefined, options: { title?: string }) => {
      if (!note) {
        console.log(
          "No note provided. Opening the system's default text editor...",
        );
        return;
      }
...
function title(program: Command): void {
  program
    .command("title")
    .description(
      "Set or overwrite the title for a given date (defaults to today).",
    )
    .argument("<title>", "The title to remember this day by.")
    .option(
      "--date <string>",
      "The date to set the title for (e.g., 2026-07-09). Defaults to today.",
    )
...
function view(program: Command): void {
  program
    .command("view")
    .description(
      "Lists all diary entries for a specific date. If no date is provided, it will show today's entries.",
    )
    .option(
      "--date <string>",
      "The date for which to display entries (e.g., 2026-07-09)",
    )
